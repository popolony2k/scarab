...
Every sprite lives on a numbered (or named) Tiled layer — layer ids are whatever you assigned them to in the Tiled editor, not something the engine invents (Caravellius keeps its own names for them in `caravellius/src/layerids.lua`).
