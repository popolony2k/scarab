...
| [Renderer](https://popolony2k.github.io/scarab/lua-api/renderer.html) | Creating the process's renderer with your own settings — `renderer_create`, `renderer_get_current`/`renderer_get_config`/`renderer_get_default_view`/`renderer_get_backend`, `renderer_destroy` — plus the `RENDERER_BACKEND_*`/`VIEW_CONTROL_MODE_*`/`FRAME_PACING_*`/`ZOOM_*` constants; includes the ordering rule (`renderer_create` must be the first window-needing call) |
| [Views](https://popolony2k.github.io/scarab/lua-api/views.html) | View-addressed zoom, viewport and camera — `view_*` (the renderer's default view is `0`; the older global `camera_*`/`zoom_*`/`viewport_*` act on it too) |
