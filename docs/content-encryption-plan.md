...
### 3. Runtime decrypt hook — Scarab's own read paths — ✅ done
- [x] `LuaEngine::RunFile`, `LuaJsonApi::LoadJson`, `LuaFileSystemApi::DoFile`, all wired
- [x] New `LuaCryptoApi::TryDecryptBytes` (`src/lua/luacryptoapi.h`/`.cpp`) — NOT Lua-bound, a plain C++ helper shared by every read path: tries to decrypt whatever raw bytes `SunLight::FileSystem::ReadFile()` returned using this build's own `SCARAB_CONTENT_KEY`, returning `false` (not an error — the normal, expected outcome for plaintext) on no key configured, a too-short blob, or an AEAD authentication failure; callers silently fall back to treating the bytes as plaintext. `crypto_decrypt_data` (the Lua-bound primitive) refactored to share the same underlying `DecryptCore` rather than duplicating the nonce/ciphertext-split-and-`crypto_secretbox_open_easy` logic a second time
- [x] **A fourth, easy-to-miss read site found and fixed while verifying end-to-end**: `EngineHost::ResolveEntryScript`'s own `.zip` branch reads a packed bundle's `project.json` directly in C++ (never through `load_json`/`LuaJsonApi`), so it needed this exact same hook too — without it, a real packed `.zip`'s own `main_script` resolution failed outright (confirmed live: `Error parsing project file (missing or invalid "main_script")`) even though `LuaEngine::RunFile`/`LuaJsonApi::LoadJson`/`LuaFileSystemApi::DoFile` were all already correctly wired. Fixed the same way as the other three sites
- [x] **Deliberately NOT applied** to the loose (non-`.zip`) `project.json` read path (`ResolveEntryScript`'s `std::ifstream` branch) — nothing in this feature's actual workflow ever produces a standalone encrypted `project.json` outside a `.zip` (`tools/pack.lua` always produces a `.zip`), so this was scoped out rather than added speculatively
- [x] Verified for real, end-to-end: a genuine packed `.zip` (entry script + `dofile`'d helper script + `load_json`'d config, all encrypted by `tools/pack.lua`) built with `scarab --pack`, then run directly (`scarab game.zip`) against a real-key build — entry script, `dofile`, and `load_json` all transparently decrypted and ran correctly (`ALL CHECKS PASSED`, no stderr warnings)
- [x] Backward compatibility (checkpoint 5's core requirement) confirmed as a side effect of this same test: the identical source files zipped **without** encryption ran correctly against the same real-key build, every read path falling back to plaintext transparently
- [x] Regression-checked against a default (empty-key) build: an existing sample (`samples/json`) still runs unaffected; a genuinely encrypted `.zip` run against a build with no key fails cleanly (a normal, actionable JSON-parse-style error - no crash, no garbled output, no undefined behavior) rather than silently misbehaving
- [x] Doc comments updated (`load_json`/`dofile`'s own `@luadoc`) to mention the transparent decrypt behavior; full doc generator run confirmed clean (`TryDecryptBytes` itself needs no `@luaname` tag — it's never `lua_register()`'d, so the generator's scanner never sees it)
- [x] **Known, accepted limitation**: a blob encrypted for a *different* key is indistinguishable from plaintext to `TryDecryptBytes` (AEAD's own guarantee - no oracle for "right ciphertext, wrong key"), so a wrong-key scenario surfaces as an ordinary Lua/JSON parse error on the still-encrypted bytes, not a dedicated "wrong key" message - acceptable for this checkpoint's scope, not revisited

### 4. Runtime decrypt hook — `sunlight`'s own read paths (cross-repo) — ✅ done
- [x] Precise spec written and verified against sunlight's actual current source (not assumed) before hand-off — the three confirmed `ReadFile()` call sites (`RaylibEngine`'s `SetLoadFileDataCallback` for textures, `RayLibSound::Load` for sounds, `TileMapRenderer::LoadMap` for the `.tmx` XML) all reach one concrete implementation, `PhysFsFileSystem::ReadFile` — proposed patching *there* instead of at the three call sites individually, since checkpoint 3 had just demonstrated that per-call-site patching misses sites (the `ResolveEntryScript` `.zip`-branch gap)
- [x] Design question resolved: a generic, crypto-agnostic `IFileSystem::SetReadFilter( ReadFilterCallback )` hook (`typedef std::function<bool(const std::vector<unsigned char>&, std::vector<unsigned char>&)>`) — sunlight itself gains zero crypto dependency or opinion on what a registered callback does; unset (the default) is a byte-identical no-op
- [x] Implemented by a sunlight-focused session exactly as proposed (one style correction: `typedef` instead of `using`, matching that codebase's existing `SoundFactory::CreatorFunction`/`InputHandlerFactory::CreatorFunction` convention) — shipped as [sunlight v0.18.0](https://github.com/popolony2k/sunlight/releases/tag/v0.18.0) (PR #93), with 5 new regression tests (`tests/test_physfsfilesystem.cpp`: no-filter passthrough, filter substitutes, filter declines, filter never invoked on a failed read, and an empty callback clearing a previously-set one) — full sunlight suite 104/104
- [x] `SCARAB_SUNLIGHT_VERSION` bumped to `0.18.0` (root `CMakeLists.txt`)
- [x] `main.cpp` registers `LuaCryptoApi::TryDecryptBytes` as the `SetReadFilter` callback once, right after `FileSystemFactory::GetFileSystem().Init()` and before any `Mount()` call, so it's in effect for the very first read — unconditional (no `SCARAB_CONTENT_KEY` check needed at the call site itself), since `TryDecryptBytes` already silently returns `false`/no-op when this build has no key configured
- [x] Verified for real, end-to-end, with genuine assets (not synthetic test data): the shared `resources/` tree (`resources/tilemap/test.tmx` + all 8 of its external `.tsx` tilesets + every tileset image, `resources/sprites/sunny_idle_down.png`, `resources/sounds/stage_theme.wav` — 31 files total) packed fully encrypted via `scarab --pack`, then run directly against a real-key build — `tilemap_load_map` (including every external tileset image, resolving the open question of whether external `.tsx` tileset image resolution routes through the same hooked path — confirmed live: it does), `sprite_configure_texture`, and `sound_load` all transparently decrypted and loaded correctly (`ALL CHECKS PASSED`, real raylib `TEXTURE`/`IMAGE`/`WAVE` trace lines for every asset, no stderr warnings)
- [x] Backward compatibility confirmed again at this layer too: the identical asset tree zipped **without** encryption ran correctly against the same real-key build
- [x] Regression-checked against a default (empty-key) build: an existing sample (`samples/tilemap`) unaffected; the same plain (unencrypted) `.zip` still works; the genuinely encrypted `.zip` fails cleanly (the same JSON-parse-style error checkpoint 3 already established, no crash) rather than misbehaving silently
- [x] Result reported back to the sunlight session that implemented it
